...
class RootIndex extends React.Component {
  render() {
    ...
    const inContactBox1 = addNewPage.contact[0].fields.textContact;
    const inContactBox2 = addNewPage.contact[1].fields.title.en_US; // 從第一個元素中獲取 textContact
    ...
    const headingContents = addNewPage.heading.map((item) =>
      item.fields.content.en_US
    );
    const contactBox = addNewPage.contact.map((item) =>
      item.fields.title.en_US
    );
    return (
      <Layout location={this.props.location}>
        <Seo
          title={addNewPage.seo.fields.metaTitle.en_US}
          description={addNewPage.seo.fields.metaDescription.en_US}
              ...
              <div className="pt-2 md:pt-3 2xl:pt-0">
                ...
                {headingContents.map((content, index) => {
                  if (index === 0) {
                    ...
                    return <p className="py-5 md:py-3 text-lg md:text-lg 2xl:text-2xl">{content}</p>;
                  } else {
                    return null;
                  }
                })}
        ...
        <section className={styles.sec2_wrap}>
          <div className="grid grid-rows-0 relative container mx-auto">
            <h2 className="flex items-center text-2xl pt-12 ml-2 font-bold">
              <span><FaSquare /></span>
              {contactBox.map((title, index) => {
                if (index === 0) {
                  return <b className="ml-3">{title}</b>;
                } else {
                  return null;
                }
              })}
            </h2>
            <h2 className="text-2xl md:text-4xl py-3 ml-0 md:ml-4 leading-0 md:leading-[3.5rem]">{inContactBox1.en_US[0].fields.content.en_US}</h2>
            <p className="text-xl mx-4">{inContactBox1.en_US[1].fields.content.en_US}</p>
          </div>
          <ServiceScope />
        </section>
          ...
          <div className={styles.container}>
            <div className="grid items-center shadow-none rounded-3xl">
              ...
              <div className="hidden md:inline text-center">
                <h2 className="text-4xl underline decoration-yellow-400 my-14">{inContactBox2}</h2>
              </div>
              <Clients />
            </div>
          </div>
...
export const pageQuery = graphql`
    query HomeQuery {
      ...
      allContentfulAddNewPage(filter: {contentful_id: {eq: "2vVfX03PDoKVGEDgRJgMz3"}}) {
        nodes {
          contentful_id
          id
          order
